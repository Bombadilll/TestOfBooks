...
@Component
public class BookDImp implements BookD {
    ...
    private static final Logger LOGGER = LogManager.getLogger(BookDImp.class);
    ...
    @Autowired
    public BookDImp(DataSource dataSource) {
    ...
    public void remove(Book book) {
        ...
        orderD.removeWithBookRemoval(currId);
        jdbcTemplate.update(BA_REMOVE, currId);
        jdbcTemplate.update(BA_REMOVE, currId);
        jdbcTemplate.update(BG_REMOVE, currId);
    ...
    public Set<Book> findBookByAuthor(long id) {
        LOGGER.debug("get list of all books by authorD "+id);
    ...
    private class BookMapper implements RowMapper<Book> {
        ...
        public Book mapRow(ResultSet resultSet, int i) throws SQLException {
            ...
            Book book = new Book.BookBuilder(name).id(id).description(description).price(price).authors(authorD.findAuthorsByBook(id)).genres(genreD.findGenresByBook(id)).build();
